...
get.boundary.with.tryCatch <- function(test.cutoff.eli, my.p.saf, my.p.tox){
  tryCatch(
    expr = {
      return(get.boundary(target=0.1, ncohort=10, cohortsize=3, n.earlystop = 12,
                          p.saf = my.p.saf, p.tox = my.p.tox, cutoff.eli = test.cutoff.eli,
...
for(test.cutoff.eli in c(0.7, 0.8, 0.9, 0.97,0.99)){
  target <- 0.1
  test.boundary <- NULL
  while(length(test.boundary)==0){
    my.p.saf <- runif(1,min=0,max=target-0.0000001)
    my.p.tox <- runif(1,min=target+0.0000001,max=1)
    test.boundary <- get.boundary.with.tryCatch(test.cutoff.eli, my.p.saf, my.p.tox)
  }## while
  ...
  file.name <- paste(paste("target.10_cutoff.eli", test.cutoff.eli,"total.n.30.rslt",
