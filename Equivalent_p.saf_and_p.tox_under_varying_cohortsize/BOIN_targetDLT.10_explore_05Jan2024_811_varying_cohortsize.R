...
get.boundary.with.tryCatch <- function(test.cohortsize, my.p.saf, my.p.tox){
  tryCatch(
    expr = {
      return(get.boundary(target=0.1, ncohort=10, cohortsize=test.cohortsize, n.earlystop = 12,
...
for(test.cohortsize in 4:8){
  ...
  file.name <- paste(paste("target.10_cohortsize", test.cohortsize,"ncohort.10.rslt",
