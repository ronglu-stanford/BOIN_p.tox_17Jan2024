...
get.boundary.with.tryCatch <- function(test.n.earlystop, my.p.saf, my.p.tox){
  tryCatch(
    expr = {
      return(get.boundary(target=0.1, ncohort=10, cohortsize=3, n.earlystop = test.n.earlystop,
...
for(test.n.earlystop in c(15,18,21,24,27,30)){
  target <- 0.1
  test.boundary <- NULL
  while(length(test.boundary)==0){
    my.p.saf <- runif(1,min=0,max=target-0.0000001)
    my.p.tox <- runif(1,min=target+0.0000001,max=1)
    test.boundary <- get.boundary.with.tryCatch(test.n.earlystop, my.p.saf, my.p.tox)
  }## while
  ...
  file.name <- paste(paste("target.10_earlystop.n", test.n.earlystop,"total.n.30.rslt",
